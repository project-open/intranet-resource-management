...
drop function im_rm_weekend(integer, date, date);
create or replace function im_rm_weekend (integer, date, date)
...
select im_rm_weekend(624, now()::date, '2014-06-30');



-- Returns a real[] for each day between start and end 
-- with 100 for working days and 0 for weekend + bank holidays
drop function im_rm_work_days(integer, date, date);
create or replace function im_rm_work_days (integer, date, date)
returns float[] as $body$
DECLARE
	p_user_id			alias for $1;
	p_start_date			alias for $2;
	...
	row				record;
BEGIN
	v_work_days = im_rm_weekend(p_user_id, p_start_date, p_end_date);
	...
	LOOP
		RAISE NOTICE 'im_rm_user_absence(%,%,%): Bank Holiday %', p_user_id, p_start_date, p_end_date, row.absence_name;
	...
	return v_work_days;
END;$body$ language 'plpgsql';
select im_rm_work_days(624, now()::date, '2014-06-30');


-- Returns a real[] for each day between start and end 
-- with 100 for each day the user has taken vacation.
-- Half days off will appear with 50.
drop function im_rm_user_absence(integer, date, date);
create or replace function im_rm_user_absence (integer, date, date)
returns float[] as $body$
DECLARE
	p_user_id			alias for $1;
	p_start_date			alias for $2;
	...
	row				record;
BEGIN
	v_work_days = im_rm_work_days(p_user_id, p_start_date, p_end_date);
	...
	LOOP
		...
		ELSE
			...
			RAISE WARNING 'im_rm_user_absence(%,%,%): Absence %: No workdays for vacation', p_user_id, p_start_date, p_end_date, row.absence_name;
			v_absence_percentage := 0;
		END IF;
		IF v_absence_duration_work_days > 100 THEN
			-- Error: Overlapping vacations or duration days > work days
			RAISE WARNING 'im_rm_user_absence(%,%,%): Absence %: More duration_days=% than work_days=%', p_user_id, p_start_date, p_end_date, row.absence_name, row.duration_days, v_absence_duration_work_days;
			v_absence_percentage := 100;
		END IF;

		-- Add the absence percentage to the result set
		RAISE NOTICE 'im_rm_user_absence(%,%,%): Absence %: dur=%, workdays=%, perc=%', p_user_id, p_start_date, p_end_date, row.absence_name, row.duration_days, v_absence_duration_work_days, v_absence_percentage;
	...
	return v_result;
END;$body$ language 'plpgsql';
select im_rm_user_absence(624, now()::date, '2014-06-30');
...
drop function im_rm_user_assignation(integer, date, date);
create or replace function im_rm_user_assignation (integer, date, date)
returns float[] as $body$
DECLARE
	p_user_id			alias for $1;
	p_start_date			alias for $2;
